...
def extract_citations(text: str) -> CitationResult:
    ...
    for m in footnote_pattern.finditer(text):
        ...
        existing = next((c for c in citations if c.index == n and c.kind == "numbered"), None)
        if existing:
    ...
    numbered = sorted([c for c in citations if c.index is not None], key=lambda c: c.index)
